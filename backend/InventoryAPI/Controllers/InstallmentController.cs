...
namespace InventoryAPI.Controllers
{
    ...
    public class InstallmentController : ControllerBase
    {
        ...
        {
            return Ok("Installment service is working!");
        }

        /// <summary>
        /// Clear all installment plans (for development/testing)
        /// </summary>
        [HttpDelete("clear-all")]
        public async Task<ActionResult<string>> ClearAllInstallmentPlans()
        {
            try
            {
                // This is a development endpoint to clear problematic data
                var result = await _installmentPlanService.ClearAllPlansAsync();
                return Ok($"Cleared all installment plans. Result: {result}");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error clearing installment plans");
                return StatusCode(500, "An error occurred while clearing installment plans");
            }
        }
        ...
        public async Task<ActionResult<string>> TestCreateInstallmentPlan([FromBody] CreateInstallmentPlanDto createPlanDto)
        {
            try
            {
                _logger.LogInformation("Test create endpoint called");
                
                // Create a simple plan directly
                var plan = new InstallmentPlan
                {
                    SaleId = createPlanDto.SaleId,
                    CustomerId = createPlanDto.CustomerId,
                    Products = new List<InstallmentPlanProduct>
                    {
                        new InstallmentPlanProduct
                        {
                            ProductId = createPlanDto.ProductId,
                            Name = "Test Product",
                            Price = createPlanDto.TotalPrice,
                            Quantity = 1,
                            Category = "Test",
                            Description = "Test Description"
                        }
                    },
                    TotalPrice = createPlanDto.TotalPrice,
                    DownPayment = createPlanDto.DownPayment,
                    NumberOfInstallments = createPlanDto.NumberOfMonths,
                    InstallmentAmount = (createPlanDto.TotalPrice - createPlanDto.DownPayment) / createPlanDto.NumberOfMonths,
                    InterestRate = createPlanDto.InterestRate,
                    StartDate = Convert.ToDateTime(createPlanDto.StartDate),
                    EndDate = Convert.ToDateTime(createPlanDto.StartDate).AddMonths(createPlanDto.NumberOfMonths),
                    Status = "Active",
                    Payments = new List<InstallmentPayment>(),
                    TotalPaid = createPlanDto.DownPayment,
                    RemainingBalance = createPlanDto.TotalPrice - createPlanDto.DownPayment,
                    CreatedAt = DateTime.UtcNow,
                    UpdatedAt = DateTime.UtcNow
                };

                _logger.LogInformation("Test plan created, returning success");
                return Ok($"Test plan created successfully for sale {createPlanDto.SaleId}");
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error in test create endpoint");
        ...
        public async Task<ActionResult<InstallmentPlan>> CreateInstallmentPlan([FromBody] CreateInstallmentPlanDto createPlanDto)
        {
            try
            {
                _logger.LogInformation($"Controller: Received create request for sale {createPlanDto.SaleId}");
                
                if (!ModelState.IsValid)
                {
                    _logger.LogWarning("Controller: Model state is invalid");
                    return BadRequest(ModelState);
                }

                _logger.LogInformation("Controller: Model state is valid, proceeding with creation");
