...
namespace InventoryAPI.Services
{
    public class InstallmentPlanService : IInstallmentPlanService
    {
            ...
            decimal totalPrice, decimal downPayment, int months, double interestRate, DateTime startDate)
        {
            try
            {
                _logger.LogInformation($"Creating installment plan for sale {saleId}, customer {customerId}, product {productId}");
                
                // Validate input
                if (downPayment > totalPrice)
                {
                    _logger.LogWarning("Down payment cannot be greater than total price");
                    throw new ArgumentException("Down payment cannot be greater than total price");
                }

                if (months <= 0)
                {
                    _logger.LogWarning("Number of months must be greater than 0");
                    throw new ArgumentException("Number of months must be greater than 0");
                }

                if (interestRate < 0)
                {
                    _logger.LogWarning("Interest rate cannot be negative");
        ...
        public async Task<decimal> CalculateInstallmentAmountAsync(decimal principalAmount, double interestRate, int numberOfMonths)
        {
            try
            {
                if (interestRate == 0)
                {
                    return Math.Round(principalAmount / numberOfMonths, 2);
                }

                // Flat Interest Method: Total Interest = Principal × Rate × Time
                var totalInterest = principalAmount * (decimal)(interestRate / 100);
                var totalAmount = principalAmount + totalInterest;
                
                // Equal installments
                return Math.Round(totalAmount / numberOfMonths, 2);
            }
            catch (Exception ex)
            {
            ...
            int numberOfMonths, DateTime startDate)
        {
            try
            {
                ...
                var totalInterest = principalAmount * (decimal)(interestRate / 100);
                var interestPerInstallment = Math.Round(totalInterest / numberOfMonths, 2);
                var principalPerInstallment = Math.Round(principalAmount / numberOfMonths, 2);
                ...
                for (int i = 0; i < numberOfMonths; i++)
                {
                    ...
                    var currentPrincipal = (i == numberOfMonths - 1) ? remainingPrincipal : principalPerInstallment;
                    var currentInterest = (i == numberOfMonths - 1) ? (installmentAmount - currentPrincipal) : interestPerInstallment;
        ...
        public async Task<bool> ClearAllPlansAsync()
        {
            try
            {
                return await _installmentPlanRepository.ClearAllPlansAsync();
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error clearing all installment plans");
                throw;
            }
        }
    }
